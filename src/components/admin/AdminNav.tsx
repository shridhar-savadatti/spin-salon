...
import { Calendar, Settings, BarChart2, LogOut, FileText, Megaphone, Menu, X, Tag, Users, Sparkles, Receipt, Wallet } from "lucide-react";
...
const STORE_ITEMS = [
  ...
  { href: "/admin/wallet-plans", label: "Memberships", icon: Wallet },
    ...
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between border-b border-zinc-800 bg-zinc-900 px-4 py-3 md:hidden">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-white text-sm">
            Spin <span className="text-zinc-400">Admin</span>
          </span>
          {role && (
            <span className={cn("rounded px-1.5 py-0.5 text-xs font-medium",
              role === "blog" ? "bg-blue-900 text-blue-300" : "bg-zinc-800 text-zinc-400")}>
      ...
      {open && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="absolute top-0 left-0 bottom-0 w-64 bg-zinc-900 shadow-xl pt-16">
      ...
      {/* ── Desktop sidebar ───────────────────────────────── */}
      <aside className="hidden md:flex min-h-screen w-64 flex-col border-r border-zinc-800 bg-zinc-900">
        <div className="flex items-center gap-2 border-b border-zinc-800 p-6">
          ...
          <div className="flex-1 min-w-0">
            <span className="font-bold text-white">
              Spin <span className="text-zinc-400">Unisex</span>
            </span>
            {role && (
